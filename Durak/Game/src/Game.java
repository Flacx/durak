...
public class Game {
	...
	public int executeAttack(int attackingPlayer, boolean shifted) {
        ...
        coverphase:
        while (!player1ok && !player2ok) {
            printTable();
            // p0 deckt. Szenarien: deckt und niemand wirft dazu runde vorbei. schiebt weiter -> runde vorbei. p1 und p2 legen dazu, p0 deckt / schluckt. next play
            ...
            if (answerCard == -1) {
                ...
                if ((answer.getColor() == attackingCards[attackingIndex].getColor()) && attackingCards[attackingIndex].getColor() != trumpColor && answer.getColor() != trumpColor &&(int) answer.getNumber() > (int) attackingCards[attackingIndex].getNumber()) {
                ...
                // coverKarte Farbe == trumpffarbe
                // coverKarte Nummer >
                } else if (attackingCards[attackingIndex].getColor() == trumpColor && answer.getColor() == trumpColor && (int) answer.getNumber() > (int) attackingCards[attackingIndex].getNumber()) {
                    coverCards[attackingIndex] = (Card) players[attackedPlayer].getHand().get(answerCard);
                    players[attackedPlayer].getHand().remove(answerCard);
                    players[attackedPlayer].shiftDownCards();
